...
sese_final <- sese |>
  ...
  ungroup() |>
  left_join(p |> select(Plot, SampleYear, Elevation, Slope, BurnScar, tFire), 
                        by = c("Plot", "SampleYear")) |>
  rename(CwdHostBiomassNorm = BCCWDHostBiomass,
         CwdHostBiomass = CWD_Host_Biomass,
         SeseBiomass = TotalBiomass,
         SeseBiomassNorm = BCTotalBiomass,
         UmcaStemDensity = UmcaTotalStem,
...
sese_pa <- sese_final |>
  mutate(across(c("PRam", "Fire"), as.numeric)) |> distinct()

sese_mixed <- sese_final |> distinct()
...
intro <- "SeseBiomassNorm ~ CwdHostBiomassNorm + LideSnagBiomassNorm + Fire + PRam + Elevation + Slope + tFire
...
sese_final$ElevationNorm <- box_cox(sese_final$Elevation)
sese_final$SlopeNorm <- box_cox(sese_final$Slope)
sese_final$tFireNorm <- box_cox(sese_final$tFire)
...
comp_me(m1 = fit, m2 = bio_mixed, m3 = model_twoway, title1 = "Path analysis", title2 = "Mixed effects model", title3 = "Fixed effects model")
