...
q_sums1 <- q_sums |>
  arrange(Plot, SampleYear, Status, PRam, Fire) |>          
  group_by(Plot, Species, Status, PRam, Fire) |>
  mutate(DeltaBiomass = TotalBiomass - lag(TotalBiomass))
...
snag_host <- snag_host |> select(Plot, SampleYear, LideSnagStemDensity, LideSnagBiomass, LideSnagBiomassNorm)
...
m$CwdHostBiomassNorm <- box_cox(m$CWD_Host_Biomass) 
...
cwd_year <- cwd_year |>
  mutate(DiseaseFuels = LideSnagBiomass + CWD_Host_Biomass,
         DiseaseFuelsNorm = LideSnagBiomassNorm + CWD_Host_Biomass
)
...
ts_look(q_sums, "LIDE", "TotalBiomass")
...
## Disease and fire impacts on living biomass  
...
pa <- function(model, df, species, title, export = FALSE) {
  ...
  df <- df |>
    filter(Species == {{species}} & Status == 0) 
...
pa(model = "TotalBiomassNorm ~ LideSnagBiomassNorm + Fire + PRam + ElevationNorm + SlopeNorm + tFireNorm
          LideSnagBiomassNorm ~ UmcaStemDensityNorm + PRam 
          ", df = all_year, species = "SESE", title = "Path analysis quantifying CWD fuels as a mediator for disease driven fire impacts to Redwood biomass",
          export = FALSE)
...
pa(model = "TotalBiomassNorm ~ DiseaseFuelsNorm + Fire + PRam + ElevationNorm + SlopeNorm + tFireNorm
          DiseaseFuelsNorm ~ UmcaStemDensityNorm + LideSnagBiomassNorm + PRam + Fire + tFire
          ", df = cwd_year, species = "SESE", title = "Path analysis quantifying CWD fuels as a mediator for disease driven fire impacts to Redwood biomass",
          export = FALSE)
...
bio_mixed <- lmer(
                  SeseBiomassNorm ~ CwdHostBiomassNorm + LideSnagBiomassNorm + Fire + PRam + ElevationNorm + SlopeNorm  + tFire +
                    (1 | Plot) + (1 |SampleYear),
                  data = sese_final)
...
sese_final$predictedSeseBiomass <- predict(bio_mixed, newdata = sese_final, 
...
c <- sese_final |>
  ...
  geom_point() +
  geom_smooth_3d(aes(z = predictedSeseBiomass),
...
```{r}
comp_me <- function(m1, m2, title1, title2, m3, title3) {
  df <- data.frame(
                   Model = c(title1, title2, title3),
...
comp_me(m1 = fit_sa, m2 = bio_mixed, m3 = model_twoway, title1 = "Path analysis", title2 = "Mixed effects model", title3 = "Fixed effects model")
